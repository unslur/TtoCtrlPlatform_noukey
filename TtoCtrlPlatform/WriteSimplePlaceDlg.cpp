...
BOOL CWriteSimplePlaceDlg::OnInitDialog()
{
	...
	if(iter.HasMore()){
		...
		string offdocNum = (const char*)iter.GetData(3);
		if("" != offdocNum)
		{
			m_EoffDocNum.SetWindowText(offdocNum.c_str());
		}
		string standard = (const char*)iter.GetData(4);
		if("" != standard)
		{
			m_Estandard.SetWindowText(standard.c_str());
		}
		string def1 = (const char*)iter.GetData(5);
		if("" != def1)
		{
			m_Edef1.SetWindowText(def1.c_str());
		}
		string def2 = (const char*)iter.GetData(6);
		if("" != def2)
		{
			m_Edef2.SetWindowText(def2.c_str());
		}
		string def3 = (const char*)iter.GetData(7);
		if("" != def3)
		{
