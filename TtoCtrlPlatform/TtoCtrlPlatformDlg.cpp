	...
	try{
		...
		while(iter.HasMore()){
			memset(tmpTaskId,0,sizeof(tmpTaskId));
				...
				string itemnum = (const char*)iter.GetData(11);//区分中包字段
...
BOOL CTtoCtrlPlatformDlg::OnInitDialog(){
	...
	string sql_init="select orgId,taskServerIp,taskServerPort,httpServerIp,httpServerPort,comPort,cpyNameSim,timeType,weightType from table_login where loginname='"+ttoLogin.loginname+"'";
	//===================================================
	iter = db.ExeSql(sql_init.c_str());
	if(iter.HasMore()){
...
///////////////////////////
	...
	string itemnum="";
	if(iter.HasMore()){
		itemnum = (const char*)iter.GetData(1);
...
BOOL CTtoCtrlPlatformDlg::GetSimplePlaceFromDB(string inTaskId,string &sPlace,string &sCode1)
{
	...
	iter = db.ExeSql(sqlBuff);
	if(iter.HasMore()){
		sPlace = (const char*)iter.GetData(0);
		sCode1 = (const char*)iter.GetData(1);
		return TRUE;
	}
	return FALSE;
}
...
BOOL CTtoCtrlPlatformDlg::GetSimplePlaceFromDB(string inTaskId,string &sPlace,string &sCode1,string &sPec)
{
	...
	if(iter.HasMore()){
		sPlace = (const char*)iter.GetData(0);
		sCode1 = (const char*)iter.GetData(1);
		sPec = (const char*)iter.GetData(2);
		return TRUE;
	}
	return FALSE;
}
...
bool print2( int index,int location=3){
	...
	if (strstr(printerName, "POSTEK") != NULL)
	{
		...
		int	rtn = printer.PrintPsk2(NULL, code2dStr, location);
		if(rtn==0){
				return false;
				}		
		printer.PskUnInit();
...
bool checkPrinter(){
	...
	if (strstr(printerName, "POSTEK") != NULL)
	{
		...
		//rtn = printer.PrintPsk2(NULL, printStr, location);
		printer.PskUnInit();
	}else{
	return false;
	}
...
bool print3(int index,int location=4){
	char printerName[0x64] = {0};
		strcpy(printerName,GetDefaultPrinter().c_str());
	if (strstr(printerName, "POSTEK") != NULL)
	{
		...
		int rtn = printer.PrintPsk3(NULL, code2dStr, location);
		if(rtn==0){
			return false;
		}
		printer.PskUnInit();
...
BOOL CTtoCtrlPlatformDlg::NormalPrint2(const char *code2d,int index,bool firstFlag,int& retFlag){
	...
	if(!firstFlag){		
		if(codeNum==max){
			code2dStr+="*CPC*ZYCZS"+code+"\n溯源码:"+code+"\n品 名:"+deviceArr[index]->medName+"\n重 量:"+deviceArr[index]->medWeight+"\n产 地:"+deviceArr[index]->medPlace+"\n生产日期:"+deviceArr[index]->medDate+"\n批 号:"+deviceArr[index]->medBatch+"\n厂 家:"+allUseNameSim+"\n";
			if(!print2(index)){
			return false;
			}
		...
		if(code2dStr.length()>10){
			if(!print2(index,codeNum==max?max+1:codeNum)){
			return false;
			}
		}
...
BOOL CTtoCtrlPlatformDlg::NormalPrint3(const char *code2d,int index,bool firstFlag,int& retFlag){
	static int codeNum=0;
	int max=3;
	string code=code2d;
	codeNum++;
if(!checkPrinter()){
	...
	if(!firstFlag){		
		if(codeNum==max){
			code2dStr+="*CPC*ZYCZS"+code+"\n溯源码:"+code+"\n"+deviceArr[index]->medName+" "+deviceArr[index]->medWeight+"\n"+deviceArr[index]->medPlace+"\n"+deviceArr[index]->medDate+"\n批号:"+deviceArr[index]->medBatch+"\n厂家:"+allUseNameSim+"\n";
			if(!print3(index)){
				codeNum=0;
			return false;
			}
...
BOOL CTtoCtrlPlatformDlg::NormalPrint3_test(const char *code2d,int index,bool firstFlag,int& retFlag){
	static int codeNum=0;
	int max=3;
	string code=code2d;
	codeNum++;
...
void CTtoCtrlPlatformDlg::testPrint(){
	string code="52345678908234567890123456789011";
	int index=0;
	
	int ret=0;
	for(int i=0;i<5;i++){
		NormalPrint3_test(code.c_str(),index,false,ret);	
	}
	NormalPrint3_test("",index,true,ret);
	for(int i=0;i<4;i++){
		NormalPrint3_test(code.c_str(),index,false,ret);	
	}
	NormalPrint3_test("",index,true,ret);
...
BOOL CTtoCtrlPlatformDlg::GetCodeAndPrint(int index,CTtoCtrlPlatformDlg* tto){
	...
	while (!deviceArr[index]->oneStopFlag && !workFlag && deviceArr[index]->printedNum < deviceArr[index]->medPacketsNum)
	{
		...
		while (deviceArr[index]->iter.HasMore())
		{
			...
			}else if(3==deviceArr[index]->deviceType){
				if(!NormalPrint2(sqlBuff,index,printFlag++<0,retFlag)){
					if (!workFlag) {
						if(1 == retFlag) tto->ListDevice.SetItemText(index, 4, "打印机设备打印异常,请检查是否设置为默认打印机或者是否安装打印机驱动！");
						else if(2 == retFlag) tto->ListDevice.SetItemText(index, 4, "打印机设备打印超时！");
					}
					printFlag = -2;
					memset(sqlBuff,0,sizeof(sqlBuff));
					sprintf_s(sqlBuff,512,"update table_task Set tabletPrintedNum = %d where tabletstorid =%s",deviceArr[index]->printedNum,deviceArr[index]->TaskId.c_str());
			...
			else if(4==deviceArr[index]->deviceType){
				if(!NormalPrint3(sqlBuff,index,printFlag++<0,retFlag)){
					if (!workFlag) {
						if(1 == retFlag) tto->ListDevice.SetItemText(index, 4, "打印机设备打印异常,请检查是否设置为默认打印机或者是否安装打印机驱动！");
						else if(2 == retFlag) tto->ListDevice.SetItemText(index, 4, "打印机设备打印超时！");
					}
		...
		if(3==deviceArr[index]->deviceType||4==deviceArr[index]->deviceType){
			if(3==deviceArr[index]->deviceType){
					NormalPrint2("",index,true,retFlag);
			}else if(4==deviceArr[index]->deviceType){
					NormalPrint3("",index,true,retFlag);
		}
			workFlag=false;
		}
	}
...
unsigned int WINAPI CTtoCtrlPlatformDlg::PrintThread(void* lpParam){  
	...
	try{
		...
		deviceArr[index]->medName = (const char*)deviceArr[index]->iter.GetData(0);
		if (2 == tto->weightType)
		{
			deviceArr[index]->medWeight = (const char*)deviceArr[index]->iter.GetData(1);
		}
		...
		{
			string strwei = (const char*)deviceArr[index]->iter.GetData(1);
			deviceArr[index]->medWeight = strwei + "g";
		}
		deviceArr[index]->medSpecial = (const char*)deviceArr[index]->iter.GetData(2);
		deviceArr[index]->medPlace = (const char*)deviceArr[index]->iter.GetData(7);
		deviceArr[index]->medPlaceAll = (const char*)deviceArr[index]->iter.GetData(3);
		deviceArr[index]->medBatch = (const char*)deviceArr[index]->iter.GetData(4);
		deviceArr[index]->medDate = (const char*)deviceArr[index]->iter.GetData(5);
		...
		deviceArr[index]->medDef1 = (const char*)deviceArr[index]->iter.GetData(9);
		deviceArr[index]->medDef2 = (const char*)deviceArr[index]->iter.GetData(10);
		deviceArr[index]->medDef3 = (const char*)deviceArr[index]->iter.GetData(11);
		deviceArr[index]->meditemnum = (const char*)deviceArr[index]->iter.GetData(12);
		string tmpDateTo = (const char*)deviceArr[index]->iter.GetData(13);
		if (1 == tto->timeType)
		{  
		...
		{   
			...
			deviceArr[index]->medDateTo = tmpDateTo.substr(0,4)+tmpDateTo.substr(5,2)+tmpDateTo.substr(8,2);
		deviceArr[index]->medPiwenNum = (const char*)deviceArr[index]->iter.GetData(14);
		deviceArr[index]->medStandard = (const char*)deviceArr[index]->iter.GetData(15);
